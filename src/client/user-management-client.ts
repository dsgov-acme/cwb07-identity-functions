...
export type ProfileLink = {
    ...
    id: string;
    accessLevel: 'ADMIN' | 'WRITER' | 'READER' | 'AGENCY_READONLY'; 
}
...
class UserManagementClient {
    ...
    async getUserByExternalId(identityProvider: string, externalId: string, includeAppRoles: boolean, includeUserProfiles: boolean): Promise<UserResponse | null> {
        const includeApplicationRoles = `${includeAppRoles}`
        const includeProfiles = `${includeUserProfiles}`;
        const paramters: Record<string, string> = { identityProvider, externalId, includeApplicationRoles, includeProfiles };
        const response = await fetch(`${config.userManagementBaseUrl}/api/v1/users?${new URLSearchParams(paramters)}`, {
            method: 'GET',
            headers: {
                'Accept': 'application/json',
